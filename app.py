...
# %%
# Function to fetch data från Socrata API
def fetch_cftc_data(contract_code, start_date, end_date, limit=5000):
    """
    Fetches data from CFTC:s Socrata API baserat på kontraktskod och datumintervall.
    ...
    :param limit: (int) Max number of rows to fetch (default 5000)
...
# Define the ticker
    ...
    # Add Date column med endast datum (ingen tid)
    silver_data["Date"] = silver_data.index.normalize()

    # Filter based on desired number of years (igen, för säkerhets skull)
...
silver_df = fetch_silver_price_data(years=2)
...
def load_or_update_data(force_update=False, refresh_interval_hours=4):
    ...
    data_path = "data/silver_dashboard.parquet"
    ...
    if not force_update and os.path.exists(data_path) and os.path.exists(timestamp_path):
        ...
        if datetime.now() - last_update < timedelta(hours=refresh_interval_hours):
            print(f"✅ Using cached data (uppdaterad {last_update.strftime('%Y-%m-%d %H:%M')})")
